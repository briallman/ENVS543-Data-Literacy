...
In this analysis, several steps will be undertaken to examine sex ratio divergence among *Araptus attenuatus* populations in relation to habitat suitability.

First, the coordinates (longitude and latitude) of each site will be extracted from the `locations` dataset. These coordinates will be used to extract habitat suitability values from the `suitability_now` and `suitability_lgm` raster datasets for both current and historical conditions.

Next, the sex ratios for each site will be calculated by aggregating the data from the `samples` dataset. For each site, the number of males and females will be summed, and the corresponding habitat suitability value for each site will be retrieved by matching the site names to the suitability data. The sex ratio will be computed as the proportion of males to the total number of individuals at each site. Sites with missing suitability data or a total of zero individuals will be excluded from further analysis.

To explore the relationship between sex ratio and habitat suitability, a generalized linear model (GLM) with a binomial family will be fitted, where the sex ratio will be modeled as a function of the current habitat suitability (`suitability_now`). This model will be used to predict the sex ratio for each site under historical conditions using the `suitability_lgm` values. The predicted sex ratios will be transformed using the logistic function to obtain estimates of sex ratios under the historical habitat conditions.

For visualization, scatter plots will be created to show the current and historical sex ratios against habitat suitability. Linear regression lines will be included on these plots to illustrate any trends. Both plots will be displayed together for comparison. Additionally, a summary table of the sex ratios for each site across the two time periods will be created, highlighting the differences in sex ratios between the current and historical periods.

Finally, habitat suitability maps will be generated by converting the raster data for both current and historical suitability into data frames using the `rasterToPoints` function. These data frames will be used to create spatial maps of habitat suitability across the study area. The maps will be visualized using `ggplot2`, with color gradients representing the variation in habitat suitability.

By following these methods, the analysis will assess how changes in habitat suitability have influenced sex ratio divergence among *Araptus attenuatus* populations.
